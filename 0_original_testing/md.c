				...
				for (int a = -1; a <= 1; a++) {
					for (int b = -1; b <= 1; b++) {
						...
						while (q != NULL) {
							...
							// if (i == 8 && j == 1 & k == 0 && a == 0 && b == 0 && (c == 4 || c == 3) && moved == 1){
							// 	printf("p0 (8, 1) real coords: (%lf, %lf)\n", p_real_x, p_real_y);
							// 	printf("p%d (8, 1) real coords: (%lf, %lf)\n", c, q_real_x, q_real_y);
							// }
							...
							// if (i == 8 && j == 1 & k == 0 && a == 0 && b == 0 && (c == 4 || c == 3) && moved == 1){
							// 	printf("p0 -> p%d (8, 1) distance - x: %lf, y: %lf, abs: %lf)\n", c, dx, dy, r_2);
							// }
							...
							if (r_2 < r_cut_off_2) {
								...
								// if (i == 8 && j == 1 & k == 0 && a == 0 && b == 0 && (c == 4 || c == 3) && moved == 1){
								// 	printf("p0 cell (8, 1) with p%d cell (%d, %d)\n", c, i+a, j+b);
								// 	printf("force = (48 * %lf * %lf * (%lf - 0.5)) = %lf\n", r_2_inv, r_6_inv, (r_6_inv), f);
								// 	printf("new ax: %lf\n", f*dx);
								// 	printf("new ay: %lf\n", f*dy);
								// 	printf("\n");
								// }
							...
							// if (i == 8 && j == 1 && moved == 1){
							// 	// struct particle_t* p = &current_cell.particles[0];
							// 	printf("p0 in cell (8, 1) after p%d in cell (%d, %d) - x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", c, i+a, j+b, p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
							// }
							q = q->next;
							c++;
						}
		...
		for (int j = 1; j < y+1; j++) {
			...
			while (p != NULL) {
				...
				if ((p->x < 0.0) | (p->x >= cell_size) | (p->y < 0.0) | (p->y >= cell_size)) {
					...
					// if (i == 7 && j == 1){
					// 	printf("p->x = p->x + (x_shift * -cell_size) = %lf + (%lf * -%lf) = p->x + %lf\n", p->x, x_shift, cell_size, (x_shift * -cell_size));
					// 	printf("p->y = p->y + (y_shift * -cell_size) = %lf + (%lf * -%lf) = p->y + %lf\n", p->y, y_shift, cell_size, (y_shift * -cell_size));
					// }
					p->x = p->x + (x_shift * -cell_size);
					p->y = p->y + (y_shift * -cell_size);

					if (i == 282 && j == 190 && count == 827){
						// struct particle_t* q = cells[282][190].head;
						// int b = 0;
						// while (q != NULL){
						// 	printf("particle p%d in (282, 190) before moving particles after %d iter:\n", b, count);
						// 	printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", q->x, q->y, q->ax, q->ay, q->vx, q->vy, q->part_id);
						// 	q = q->next;
						// 	b++;
						// }
						// printf("\n");
					...
					if (i == 282 && j == 190 && count == 827){
						printf("moved p%d from (%d, %d) to (%d, %d)\n", a, i, j, new_i, new_j);
						// struct particle_t* q = cells[282][190].head;
						// int b = 0;
						// while (q != NULL){
						// 	printf("particle p%d in (282, 190) after moving particles after %d iter:\n", b, count);
						// 	printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", q->x, q->y, q->ax, q->ay, q->vx, q->vy, q->part_id);
						// 	q = q->next;
						// 	b++;
						// }
...
int main(int argc, char *argv[]) {
	...
	// struct particle_t* p = cells[7][1].head;
	// printf("initial p0 in cell (7, 1)\n");
	// printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
	// printf("\n");

	// printf("intial particles cell (8, 1):\n");
	// struct particle_t* p = cells[8][1].head;
	// while (p != NULL) {
	// 	printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
	// 	p = p->next;
	// }
	...
	for (t = 0.0; t < t_end; t+=dt, iters++) {
		...
		// printf("particles in cell (282, 190) before update_cells in iter %d:\n", iters);
		if (iters == 827){
			struct particle_t* p = cells[282][190].head;
			int i = 0;
			while (p != NULL){
				printf("particle p%d in (282, 190) before update_cells after %d iter:\n", i, iters);
				printf("p%d - x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", i, p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
				p = p->next;
				i++;
			}
			printf("\n");
		...
		if (iters == 827){
			...
			int i = 0;
			while (p != NULL){
				printf("particle p%d in (282, 190) after update_cells after %d iter:\n", i, iters);
				printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
				p = p->next;
				i++;
			}
		...
		// if (iters+1 == 10 || iters+1 == 11 || iters+1 == 12){
		// 	struct particle_t* p = cells[7][1].head;
		// 	printf("particle p0 in (7, 1) after comp_accel after %d iter:\n", iters+1);
		// 	printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
		// 	printf("\n");
		// }
		...
		// if (moved == 1){
		// 	struct particle_t* p = cells[7][1].head;
		// 	printf("particle (7, 1) after %d iter:\n", iters+1);
		// 	printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
		// 	printf("\n");
		// 	exit(1);
		// }
		...
		if (iters % output_freq == 0) {
			...
			// struct particle_t* p = cells[2][2].head;
			// printf("particle (2, 2):\n");
			// printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
			// printf("\n");

			// printf("particles cell (8, 1) after %d iter:\n", iters+1);
			// struct particle_t* p = cells[8][1].head;
			// while (p != NULL) {
			// 	printf("x: %lf, y: %lf, ax: %lf, ay: %lf, vx: %lf, vy: %lf, part_id: %d\n", p->x, p->y, p->ax, p->ay, p->vx, p->vy, p->part_id);
			// 	p = p->next;

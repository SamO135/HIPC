...
void add_particle(struct cell_list * list, struct particle_t * particle) {
	list->num_particles++;
	list->particles = realloc(list->particles, list->num_particles * sizeof(struct particle_t));

	//check if realloc was successful
	if (list->particles == NULL){
		fprintf(stderr, "Memory reallocation failed in add_particle\n");
		exit(EXIT_FAILURE);
	}
...
void remove_particle(struct cell_list * list, int index) {
	list->num_particles--;
	if (list->num_particles == 0){
		free(list->particles);
		list->particles = calloc(1, sizeof(struct particle_t));
	}
	else{
		#pragma omp parallel
		...
		list->particles = realloc(list->particles, list->num_particles * sizeof(struct particle_t));
		if (list->particles == NULL){
			fprintf(stderr, "Memory reallocation failed in remove_particle. Old size: %lf, New size: %lf\n", ((list->num_particles+1) * sizeof(struct particle_t)), (list->num_particles * sizeof(struct particle_t)));
			exit(EXIT_FAILURE);
		}
	}
}
